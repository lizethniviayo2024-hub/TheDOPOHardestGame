...
import domain.SaveManager;
import domain.LoadManager;
...
public class GamePanel extends JPanel implements KeyListener {
	...
	public GamePanel() {
		...
		addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				// No borrar las teclas cuando se pierde el foco por abrir
				// el diálogo de pausa: si el juego está pausado queremos
				// conservar el estado de teclas para que al reanudar el
				// jugador continúe moviéndose si la tecla sigue presionada.
				if (!isPaused) {
					keysPressed.clear();
				}
			}
	...
	public void startLevelPvP(int level, String player1Name, String p1Type, Color p1Border, String player2Name,
		...
		try {
			...
			board.loadPvPMode(levelFile(level), player1Name, p1Type, p1Border, player2Name, p2Type, p2Border);
	...
	public void startLevelPvMAIRandom(int level, String playerType, Color borderColor) {
		stopTimers();
		keysPressed.clear();
		board = new Board(boardWidth(), boardHeight());
		try {
	...
	public void startLevelPvMAIExpert(int level, String playerType, Color borderColor) {
		stopTimers();
		keysPressed.clear();
		board = new Board(boardWidth(), boardHeight());
		try {
	...
	private void startTimers() {
		...
		secondTimer = new Timer(1000, e -> {
			...
			if (isPaused)
				return; // No actualizar tiempo si está pausado
	...
	private void processPlayer1Movement() {
		movePlayerFromKeys(0, keysPressed.contains(KeyEvent.VK_W), keysPressed.contains(KeyEvent.VK_S),
				keysPressed.contains(KeyEvent.VK_A), keysPressed.contains(KeyEvent.VK_D));
	}

	private void processPlayer2Movement() {
		movePlayerFromKeys(1, keysPressed.contains(KeyEvent.VK_UP), keysPressed.contains(KeyEvent.VK_DOWN),
				keysPressed.contains(KeyEvent.VK_LEFT), keysPressed.contains(KeyEvent.VK_RIGHT));
	}

	private void movePlayerFromKeys(int idx, boolean up, boolean down, boolean left, boolean right) {
		if (up && left) {
			board.movePlayer(idx, "UP_LEFT");
			return;
		}
		if (up && right) {
			board.movePlayer(idx, "UP_RIGHT");
			return;
		}
		if (down && left) {
			board.movePlayer(idx, "DOWN_LEFT");
			return;
		}
		if (down && right) {
			board.movePlayer(idx, "DOWN_RIGHT");
			return;
		}
		if (up) {
			board.movePlayer(idx, "UP");
			return;
		}
		if (down) {
			board.movePlayer(idx, "DOWN");
			return;
		}
		if (left) {
			board.movePlayer(idx, "LEFT");
			return;
		}
		if (right) {
			board.movePlayer(idx, "RIGHT");
		}
	}
	...
	// Expose toggle for external UI buttons
	public void togglePauseExtern() {
		togglePause();
	}
	...
	public void setPaused(boolean paused) {
		...
		// Si estamos reanudando, devolver foco al panel para recibir
		// eventos de teclado y ejecutar el callback de reanudar si existe.
		if (wasPaused && !isPaused) {
			requestFocusInWindow();
			if (onResume != null)
				onResume.run();
		}
		// Si se pausa vía setPaused, ejecutar callback de pausa también.
		if (!wasPaused && isPaused) {
			if (onPause != null)
				onPause.run();
		}
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_G) {
			...
			try {
				...
				SaveManager.saveGame(board, "save.txt");
		...
		if (e.getKeyCode() == KeyEvent.VK_L) {
			...
			try {
				...
				LoadManager.loadGame(board, "save.txt");
		...
		// Tecla ESC para salir al menú (terminar la partida)
		if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
			// Detener juego y notificar al callback
			stopGame();
	...
	protected void paintComponent(Graphics g) {
		...
		for (int[] c : board.getCollectibleRects()) {
			Color col = new Color(clamp(c[4]), clamp(c[5]), clamp(c[6]));
			boolean rect = c[7] == 1;
			g2.setColor(col);
			if (rect) {
				g2.fillRect(c[0], c[1], c[2], c[3]);
			} else {
	...
	private void drawPauseScreen(Graphics2D g2) {
		...
		String instructionText = "Juego pausado";
	...
	private int boardWidth() {
		return getWidth() > 0 ? getWidth() : 600;
	}

	private int boardHeight() {
		return getHeight() > 0 ? getHeight() : 400;
	}
	...
	private void showError(String msg) {
		JOptionPane.showMessageDialog(null, msg, "Error al cargar nivel", JOptionPane.ERROR_MESSAGE);
	}
	...
	public void setOnLevelComplete(Runnable r) {
		onLevelComplete = r;
	}

	public void setOnTimeOut(Runnable r) {
		onTimeOut = r;
	}

	public void setOnHudUpdate(Runnable r) {
		onHudUpdate = r;
	}
	...
	public int getDeaths() {
		return board != null ? board.getTotalDeaths() : 0;
	}

	public int getCoinsCollected() {
		return board != null ? board.getCoinsCollected() : 0;
	}
	...
	public int getTotalCoins() {
		return board != null ? board.getTotalCoins() : 0;
	}

	public int getTimeRemaining() {
		return board != null ? board.getTimeRemaining() : 0;
	}
	...
	/**
	 * Llamado externamente cuando ocurre un timeout para reiniciar el nivel. Delega
	 * a Board.resetLevel() que registra la muerte en ScoreController con el nombre
	 * de cada jugador humano — no usa addDeath() anónimo (Bug 6).
	 */
	...
	public void startLevel(int level, String playerName, String playerType, Color borderColor) {
		stopTimers();
		keysPressed.clear();
		board = new Board(boardWidth(), boardHeight());
		try {
			// necesitas un método en Board que acepte nombre
			board.loadFromFile(levelFile(level), playerName, playerType, borderColor);
		} catch (HardestGameException e) {
			showError(e.getMessage());
			return;
		}
